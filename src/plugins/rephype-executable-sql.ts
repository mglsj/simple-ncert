...
function isSqlExecutableElement(element: Element): boolean {
	if (element.tagName === "pre" && element.children.length > 0) {
		...
		if (
			...
			(child.properties.className as string).includes("language-sql") &&
...
const rehypeExecutableSql: Plugin<[], Root> = () => {
	return (ast, file) => {
		visitParents(ast, "element", (node, ancestors) => {
			if (!isSqlExecutableElement(node)) {
				return;
			}
...
export default rehypeExecutableSql;
