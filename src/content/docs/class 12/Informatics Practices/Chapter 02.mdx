...
That is why we could append the duplicate row with label R2 when
...
<Collapse title="Create CSV File">
```py exec
# Create NCERT Folder if not exists
from pathlib import Path
Path("C:/NCERT").mkdir(parents=True, exist_ok=True)

# Create CSV File
data = """RollNo, Name, Eco, Maths
1, Arnab, 18, 57
2, Kritika, 23, 45
3, Divyam, 51, 37
4, Vivaan, 40, 60
...
with open("C:/NCERT/ResultData.csv", "w") as file:
    file.write(data)
```
</Collapse>
...
We can use the `to_csv()` function to save a DataFrame to a text or csv file. For
...
<Collapse title="View resultout.csv">
```py exec
with open("C:/NCERT/resultout.csv", "r") as file:
    print(file.read())
```
</Collapse>
...
<Collapse title="View resultonly.txt">
```py exec
with open("C:/NCERT/resultonly.txt", "r") as file:
    print(file.read())
```
</Collapse>
