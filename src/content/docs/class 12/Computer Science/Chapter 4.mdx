...
4. Remove one character from the front and one character from the rear end of deque using `DELETIONFRONT` and `DELETIONREAR` we can do it.

5. Match these two removed characters. 

6. If they are same then REPEAT Step 4 and 5 till deque is empty or left with only one character, eventually string is Palindrome else stop as string is not palindrome
