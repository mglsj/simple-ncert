...
function handleError(
	...
	file: VFile,
	options: starlightMermaidOptions | undefined,
...
export interface starlightMermaidOptions
 ...
 */
const starlightMermaid: Plugin<[starlightMermaidOptions?], Root> = (
	options,
) => {
		...
		return Promise.all(promises).then(([lightResults, darkResults]) => {
			for (const [index, instance] of instances.entries()) {
				...
				if (lightResult.status === "rejected") {
					...
					// 	replacement = fromHtmlIsomorphic(lightResult.value.svg, {
					// 		fragment: true,
...
export default starlightMermaid;
